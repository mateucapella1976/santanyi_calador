...
export default function SantanyiPage({ query, variables, data, lang }: Props) {
  const { data: d } = useTina({ query, variables, data });
  const g = d?.paginaSantanyi ?? {};
  const lk = lang === 'es' ? 'Es' : lang === 'en' ? 'En' : lang === 'de' ? 'De' : 'Ca';
  const tf = (field: string) => d?.paginaSantanyi ? tinaField(d.paginaSantanyi, field) : undefined;

  const badge    = g[`heroBadge${lk}`]    || g.heroBadgeCa    || 'Instal·lació Santanyí';
  const title    = g[`heroTitle${lk}`]    || g.heroTitleCa    || 'Piscina de Santanyí';
  const desc     = g[`heroDesc${lk}`]     || g.heroDescCa     || '';
  const address  = g.heroAddress          || 'Carrer Bernat Vidal i Tomàs, 83, 07650 Santanyí';
  ...
  const servicesTitle = g[`servicesTitle${lk}`] || g.servicesTitleCa || 'Què hi ha a Santanyí';

  const mapTitle      = g[`mapTitle${lk}`]      || g.mapTitleCa      || 'Com trobar-nos';
  const addressLabel  = g[`addressLabel${lk}`]  || g.addressLabelCa  || 'Adreça';
  const fullAddress   = g.fullAddress           || 'Carrer Bernat Vidal i Tomàs, 83\n07650 Santanyí (Mallorca)';
  const email         = g.email                 || 'piscinasantanyi@algaliasport.net';
        ...
        <div className="container-site">
          <div className="max-w-3xl">
            ...
            <div className="flex flex-wrap gap-3">
              <a href={`tel:+34${phone.replace(/\s/g, '')}`} className="btn-primary">
                <svg className="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" aria-hidden="true">
        ...
        <div className="container-site">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {services.map((svc: any, i: number) => (
              <div key={i} className="card p-6 hover:shadow-md transition-shadow text-center">
            ...
            <div>
              ...
              <div className="space-y-4 text-sm">
                <div className="flex items-start gap-3 text-gray-600">
                  <svg className="w-5 h-5 text-primary-500 mt-0.5 flex-shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" aria-hidden="true">
                    <path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0 1 18 0z"/><circle cx="12" cy="10" r="3"/>
                  </svg>
                  <div>
                ...
                <div className="flex items-center gap-3 text-gray-600">
                  <svg className="w-5 h-5 text-primary-500 flex-shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" aria-hidden="true">
                    <path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07A19.5 19.5 0 0 1 4.69 12 19.79 19.79 0 0 1 1.61 3.45a2 2 0 0 1 2-2.18h3a2 2 0 0 1 2 1.72c.127.96.361 1.903.7 2.81a2 2 0 0 1-.45 2.11L8.09 8.91a16 16 0 0 0 6 6l1.27-1.27a2 2 0 0 1 2.11-.45c.907.339 1.85.573 2.81.7A2 2 0 0 1 22 16.92z"/>
                  </svg>
                  <a href={`tel:+34${phone.replace(/\s/g, '')}`} className="hover:text-primary-600 transition-colors" data-tina-field={tf('heroPhone')}>{phone}</a>
                </div>
                <div className="flex items-center gap-3 text-gray-600">
                  <svg className="w-5 h-5 text-primary-500 flex-shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" aria-hidden="true">
                    <path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z"/><polyline points="22,6 12,13 2,6"/>
                  </svg>
                  <a href={`mailto:${email}`} className="hover:text-primary-600 transition-colors break-all" data-tina-field={tf('email')}>{email}</a>
                </div>
              </div>
              <div className="mt-6">
